...
(function () {
    ...
    const I18N = {
        pt: {
            ...
            'hero.badge': 'Big Linux · BigCommunity · Multi-DE',
            'hero.title': 'Mantenha seu <span class="grad">Big Linux</span> limpo, leve e rápido',
            'hero.lead': 'O <strong>BigLinuxCleaner</strong> é um script de manutenção que remove pacotes órfãos, atalhos quebrados e resíduos do Flatpak e Snap — e ainda atualiza o menu do seu Desktop Environment automaticamente. Tudo com um único comando.',
            'hero.cta1': 'Instalar agora',
            'hero.cta2': 'Executar uma vez',
            'hero.note': 'Requer Big Linux/BigCommunity (KDE, GNOME, XFCE, Cinnamon) ·',
            ...
            'f4.title': 'Menu sempre atualizado (Multi-DE)',
            'f4.desc': 'Reconstrói o cache do seu Desktop Environment: KDE Plasma (<code>kbuildsycoca</code>, <code>krunner</code>, <code>plasmashell</code>), GNOME (<code>update-desktop-database</code>, <code>gtk-update-icon-cache</code>), XFCE (<code>xfce4-panel -r</code>), Cinnamon (<code>cinnamon-settings-daemon</code>) — para que as mudanças apareçam na hora.',
            ...
            'f7.desc': 'Detecta atalhos da Steam com ícones faltando ou de baixa resolução e baixa o ícone correto automaticamente a partir do ID do jogo na CDN da Steam.',
            ...
            's3.desc': 'Cada clique no atalho executa a limpeza completa e atualiza o menu do seu Desktop Environment automaticamente.',
            ...
            'faq.4.a': 'Sim. O script foi criado para o Big Linux/BigCommunity, mas funciona em qualquer distro baseada em Arch que use <code>pacman</code> e KDE Plasma, GNOME, XFCE ou Cinnamon. Se a ferramenta não existir (Flatpak, Snap etc.), essa parte é ignorada sem erro.',
            ...
            'faq.8.a': 'O script detecta atalhos da Steam com ícones inválidos (arquivo inexistente, vazio ou com resolução insuficiente), extrai o ID do jogo da URL <code>steam://rungameid/{id}</code> e baixa o ícone correto automaticamente da CDN da Steam. Os ícones são instalados no tema hicolor como <code>steam_icon_{id}</code> e os atalhos são atualizados.',
            ...
            'footer.col1.p': 'Script de limpeza e manutenção para Big Linux/BigCommunity: órfãos, atalhos quebrados, ícones da Steam e cache do DE (KDE, GNOME, XFCE, Cinnamon) — em um comando.',
        ...
        en: {
            ...
            'hero.lead': '<strong>BigLinuxCleaner</strong> is a maintenance script that removes orphan packages, broken shortcuts and Flatpak and Snap leftovers — and automatically refreshes your Desktop Environment menu. All with a single command.',
            'hero.cta1': 'Install now',
            'hero.cta2': 'Run once',
            'hero.note': 'Requires Big Linux/BigCommunity (KDE, GNOME, XFCE, Cinnamon) ·',
            ...
            'f4.title': 'Menu always updated (Multi-DE)',
            'f4.desc': 'Rebuilds your Desktop Environment cache: KDE Plasma (<code>kbuildsycoca</code>, <code>krunner</code>, <code>plasmashell</code>), GNOME (<code>update-desktop-database</code>, <code>gtk-update-icon-cache</code>), XFCE (<code>xfce4-panel -r</code>), Cinnamon (<code>cinnamon-settings-daemon</code>) — so changes show up right away.',
            ...
            'f7.desc': 'Detects Steam shortcuts with missing or low-resolution icons and automatically downloads the correct icon from the Steam CDN using the game ID.',
            ...
            's3.desc': 'Every click on the shortcut runs the full cleanup and refreshes your Desktop Environment menu automatically.',
            ...
            'faq.1.a': '<code>cleaner.sh</code> performs five tasks: removes orphan <code>pacman</code> packages; scans and removes broken <code>.desktop</code> shortcuts (system, user, Flatpak and Snap); removes shortcuts from uninstalled Flatpak/Snap apps; cleans Flatpak and Snap caches and leftovers; and refreshes your Desktop Environment menu and cache (KDE Plasma, GNOME, XFCE, Cinnamon).',
            ...
            'faq.4.a': 'Yes. The script was built for Big Linux/BigCommunity, but works on any Arch-based distro using <code>pacman</code> and KDE Plasma, GNOME, XFCE or Cinnamon. If a tool is missing (Flatpak, Snap, etc.), that part is skipped without errors.',
            ...
            'faq.8.a': 'The script detects Steam shortcuts with invalid icons (missing, empty or low-resolution file), extracts the game ID from the <code>steam://rungameid/{id}</code> URL and automatically downloads the correct icon from the Steam CDN. Icons are installed into the hicolor theme as <code>steam_icon_{id}</code> and shortcuts are updated.',
            ...
            'footer.col1.p': 'Cleanup and maintenance script for Big Linux/BigCommunity: orphans, broken shortcuts, Steam icons and DE cache (KDE, GNOME, XFCE, Cinnamon) — in one command.',
        ...
        es: {
            ...
            'hero.lead': '<strong>BigLinuxCleaner</strong> es un script de mantenimiento que elimina paquetes huérfanos, accesos directos rotos y residuos de Flatpak y Snap — y además actualiza el menú de tu Desktop Environment automáticamente. Todo con un solo comando.',
            'hero.cta1': 'Instalar ahora',
            'hero.cta2': 'Ejecutar una vez',
            'hero.note': 'Requiere Big Linux/BigCommunity (KDE, GNOME, XFCE, Cinnamon) ·',
            ...
            'f4.title': 'Menú siempre actualizado (Multi-DE)',
            'f4.desc': 'Reconstruye la caché de tu Desktop Environment: KDE Plasma (<code>kbuildsycoca</code>, <code>krunner</code>, <code>plasmashell</code>), GNOME (<code>update-desktop-database</code>, <code>gtk-update-icon-cache</code>), XFCE (<code>xfce4-panel -r</code>), Cinnamon (<code>cinnamon-settings-daemon</code>) — para que los cambios aparezcan al instante.',
            ...
            'f7.desc': 'Detecta accesos directos de Steam con iconos faltantes o de baja resolución y descarga automáticamente el icono correcto desde el CDN de Steam usando el ID del juego.',
            ...
            's3.desc': 'Cada clic en el acceso directo ejecuta la limpieza completa y actualiza el menú de tu Desktop Environment automáticamente.',
            ...
            'faq.4.a': 'Sí. El script fue creado para Big Linux/BigCommunity, pero funciona en cualquier distro basada en Arch que use <code>pacman</code> y KDE Plasma, GNOME, XFCE o Cinnamon. Si la herramienta no existe (Flatpak, Snap, etc.), esa parte se omite sin errores.',
            ...
            'faq.8.a': 'El script detecta accesos directos de Steam con iconos inválidos (archivo inexistente, vacío o de baja resolución), extrae el ID del juego de la URL <code>steam://rungameid/{id}</code> y descarga automáticamente el icono correcto del CDN de Steam. Los iconos se instalan en el tema hicolor como <code>steam_icon_{id}</code> y los accesos directos se actualizan.',
            ...
            'footer.col1.p': 'Script de limpieza y mantenimiento para Big Linux/BigCommunity: huérfanos, accesos directos rotos, iconos de Steam y caché del DE (KDE, GNOME, XFCE, Cinnamon) — en un comando.',
